...
**ANSWER:** 
When the pixel size is increased, the mean remains consistent. This is reasoned by the use of the "mean" parameter within the global() function to enlarge the grain size. As a result, while the original raster averages 1000 values, subsequent rasters include a greater quantity of previously averaged values, leading to the representation of a larger number of values.

In contrast, the variance decreases as the grain size increases (approaches zero). This phenomenon occurs because as the function takes the mean of the nearby pixels, the new larger pixels are the true meaning of all of the values present, thus the pixels being truncated become more and more like. 
...
**ANSWER:** 
When faced with the task of calculating new cell values during scaling up and handling categorical variables, I may choose a summary function different from the mean. For example, if each whole number signifies a unique land cover type, averaging two cell types to generate a new value wouldn't be appropriate. Rather, I would utilize the mode of cell values. This ensures that the larger grains are characterized by the most common land cover values present in the smaller grains beforehand.

Following the increase in grain size by a factor of two using a modal approach, the mean decreased from around 3 to approximately 2.2. However, upon scaling up by factors of 5 and 10, the mean raster values ascended again to about 2.55 and 2.7, gradually nearing 3, the true mean (which ideally should correspond to the most common land cover type initially). Hence, based on this limited sample size, there seems to be an initial destabilization or skew concerning how the mean changes with a modal approach. Moreover, it seems that when scaling up by a factor of 2, if four pixels contain distinct values, the new pixel or grain will assume the lowest value, elucidating the initial decline in mean, which later stabilizes. This contrasts with the findings from Challenge 1, where the mean remained constant across grain sizes.

The variance, conversely, followed a trajectory very similar to that of Challenge 1. With increasing grain sizes, variance declined rapidly. This trend reflects the principle that, with each iteration, reducing the number of pixels to one value inherently changes the potential for variance. For example, when scaling up by a factor of 2, a cluster of pixels being scaled up might have values like 1, 2, 4, and 4. By contrast, the resulting single value (4) after scaling up would exhibit less variance, as a single value inherently lacks variance. Consequently, aggregating data using either the mean (Challenge 1) or mode (Challenge 2) mitigates variation and diminishes the measure of variance.
...
**ANSWER:** 
Through the cropping of our NLCD raster, we've defined the uppermost scale boundary for assessing skink response to forest cover. If we lack land cover data beyond our defined bounds, evaluating skink response to unknown cover extending beyond those boundaries becomes inappropriate. Ultimately, acquiring data at the appropriate scales is vital for accurate response evaluation. Hypothetically, if we had land cover data for the entire planet, there would be no impediment to modeling skink response to forest cover on a global scale.
...
**ANSWER:** As expected, the correlation between variables weakens as the relative distance increases. For example, the tree cover level within a 5 km radius shows a strong correlation with that within a 4 km radius, while it lacks correlation with that within a 100 m radius.
...
**ANSWER:** When comparing the models, it becomes evident that forest cover at the 2 km scale most accurately predicts skink presence. However, the model operating on a 4 km scale displays a Delta_AICc value very similar to that of the 2 km scale, while the 3 km and 5 km models also have Delta_AICc values less than or equal to 1.58. Given that models with values between 0 and 2 suggest "substantial support," all four models exhibit high predictiveness. The order of model performances adds to the ambiguity: the best model is at 2 km, followed by 4 km, and then 3 km and 5 km. If the 2 km model truly represented the characteristic scale, one would expect 1 km or 3 km to outperform 4 km. Thus, the characteristic scale appears to lie between 2 and 5 km, but the precise critical value remains unclear at these levels.

The beta coefficients for the 2 km, 3 km, 4 km, and 5 km models are all quite similar, hovering around 10. Considering that skink presence isn't effectively predicted by forest cover within a 1 km radius, and given the likely limited territorial range of skinks due to their size, I would propose that the mechanism by which forest cover positively influences skink presence at scales of 2 km, 3 km, 4 km, and 5 km relates more to prey availability than habitat suitability. Skinks are likely to prey on invertebrates, including winged insects with larger foraging ranges, which rely on forest proximity for resources and cover. Additionally, skinks may benefit from nesting in open sandy areas conducive to burrow-building and sun basking. It's important to note the speculative nature of these assumptions, considering the limitations of my knowledge about skinks.
...
**ANSWER:** Statistical: If you have multiple variables representing the same feature at different scales, Principal Component Analysis (PCA) can be employed to condense them into a smaller set of uncorrelated principal components. These components effectively capture variance across scales in a more concise form, eliminating the need to use each individual variable. Additionally, if two covariates representing the same ecological features at different scales show no statistical correlation, I would conclude that including them in the same multiple-linear regression model should not present a problem. Both variables could contribute unique information without raising concerns about collinearity.

Biological: Different scales may represent diverse ecological processes or interactions, making their inclusion meaningful. For example, forest cover at 500 m might affect local habitat conditions, while forest cover at 5 km could reflect broader landscape connectivity. However, in some cases, from a biological perspective, if you find highly statistically correlated covariates representing the same ecological features, like 4 km and 5 km forest cover, you can reasonably infer that they essentially capture the same ecological interactions. Consequently, including both variables may not be necessary.
